 ...
 */
package bidmachine.media3.exoplayer.ima;

import static bidmachine.media3.common.AdPlaybackState.AD_STATE_AVAILABLE;
import static bidmachine.media3.common.AdPlaybackState.AD_STATE_UNAVAILABLE;
...
import bidmachine.media3.common.AdOverlayInfo;
import bidmachine.media3.common.AdPlaybackState;
import bidmachine.media3.common.AdViewProvider;
...
import bidmachine.media3.exoplayer.source.CompositeMediaSource;
...
import bidmachine.media3.exoplayer.source.ads.ServerSideAdInsertionMediaSource.AdPlaybackStateUpdater;
import bidmachine.media3.exoplayer.source.ads.ServerSideAdInsertionUtil;
...
import bidmachine.media3.exoplayer.upstream.Loader.Loadable;
import bidmachine.media3.extractor.metadata.emsg.EventMessage;
...
public final class ImaServerSideAdInsertionMediaSource extends CompositeMediaSource<Void> {
  ...
  private void setContentTimeline(Timeline contentTimeline) {
    ...
    if (isLiveStream && Objects.equals(streamRequest.getFormat(), StreamFormat.DASH)) {
      ...
      // is reported.
      adPlaybackState = ImaUtil.maybeCorrectPreviouslyUnknownAdDurations(contentTimeline, adPlaybackState);
    }
  ...
  private void invalidateServerSideAdInsertionAdPlaybackState() {
    if (!adPlaybackState.equals(AdPlaybackState.NONE) && contentTimeline != null) {
      ...
      if (Objects.equals(streamRequest.getFormat(), StreamFormat.DASH)) {
        ...
        splitAdPlaybackStates = ImaUtil.splitAdPlaybackStateForPeriods(adPlaybackState, contentTimeline);
      } else {
      ...
      if (oldPosition.adGroupIndex != C.INDEX_UNSET) {
        ...
        if (window.lastPeriodIndex > window.firstPeriodIndex) {
          if (reason == Player.DISCONTINUITY_REASON_REMOVE) {
            setAdPlaybackState(
                ImaUtil.handleAdPeriodRemovedFromTimeline(
          ...
          Pair<Integer, Integer> adGroupIndexAndAdIndexInAdGroup =
              window.isLive()
                  ? ImaUtil.getAdGroupAndIndexInLiveMultiPeriodTimeline(
    ...
    public void onAdEvent(AdEvent event) {
      ...
      long adDurationUs =
          adPeriod.durationUs != C.TIME_UNSET
              ? adPeriod.durationUs
              : ImaUtil.secToUsRounded(event.getAd().getDuration());
      setAdPlaybackState(
          ImaUtil.addLiveAdBreak(
