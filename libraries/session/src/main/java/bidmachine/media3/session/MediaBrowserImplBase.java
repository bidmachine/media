...
/* package */ class MediaBrowserImplBase extends MediaControllerImplBase
  ...
  void notifySearchResultChanged(
      String query, int itemCount, @Nullable MediaLibraryService.LibraryParams libraryParams) {
    if (!isConnected()) {
      return;
    }
    getInstance()
  ...
  void notifyChildrenChanged(
      String parentId, int itemCount, @Nullable MediaLibraryService.LibraryParams libraryParams) {
    if (!isConnected()) {
      return;
    }
    getInstance()
  ...
  private <V> ListenableFuture<LibraryResult<V>> dispatchRemoteLibrarySessionTask(
    ...
    if (iSession != null) {
      SequencedFuture<LibraryResult<V>> result =
          sequencedFutureManager.createSequencedFuture(LibraryResult.ofError(
              LibraryResult.RESULT_INFO_SKIPPED));
      try {
        task.run(iSession, result.getSequenceNumber());
      } catch (RemoteException e) {
        Log.w(TAG, "Cannot connect to the service or the session is gone", e);
        sequencedFutureManager.setFutureResult(
            result.getSequenceNumber(), LibraryResult.ofError(
