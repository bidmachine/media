 ...
 */
package bidmachine.media3.exoplayer.hls.playlist;
...
import bidmachine.media3.common.DrmInitData.SchemeData;
...
import bidmachine.media3.common.util.UriUtil;
import bidmachine.media3.common.util.Util;
import bidmachine.media3.exoplayer.hls.HlsTrackMetadataEntry;
import bidmachine.media3.exoplayer.upstream.ParsingLoadable;
import bidmachine.media3.extractor.mp4.PsshAtomUtil;
...
public final class HlsPlaylistParser implements ParsingLoadable.Parser<HlsPlaylist> {
  ...
  private static HlsMultivariantPlaylist parseMultivariantPlaylist(
    ...
    ArrayList<HlsMultivariantPlaylist.Rendition> videos = new ArrayList<>();
    ArrayList<HlsMultivariantPlaylist.Rendition> audios = new ArrayList<>();
    ...
    while (iterator.hasNext()) {
      ...
      if (line.startsWith(TAG_DEFINE)) {
        ...
        HlsMultivariantPlaylist.Variant variant =
            new HlsMultivariantPlaylist.Variant(
                uri, format, videoGroupId, audioGroupId, subtitlesGroupId, closedCaptionsGroupId);
        variants.add(variant);
        @Nullable ArrayList<HlsTrackMetadataEntry.VariantInfo> variantInfosForUrl = urlToVariantInfos.get(uri);
    ...
    for (int i = 0; i < mediaTags.size(); i++) {
      ...
      switch (parseStringAttr(line, REGEX_TYPE, variableDefinitions)) {
        case TYPE_VIDEO:
          @Nullable HlsMultivariantPlaylist.Variant variant = getVariantWithVideoGroup(variants, groupId);
          if (variant != null) {
          ...
          if (uri == null) {
            ...
            videos.add(new HlsMultivariantPlaylist.Rendition(uri, formatBuilder.build(), groupId, name));
          }
          break;
        case TYPE_AUDIO:
          ...
          if (uri != null) {
            formatBuilder.setMetadata(metadata);
            audios.add(new HlsMultivariantPlaylist.Rendition(uri, formatBuilder.build(), groupId, name));
        ...
        case TYPE_SUBTITLES:
          ...
          if (uri != null) {
            subtitles.add(new HlsMultivariantPlaylist.Rendition(uri, formatBuilder.build(), groupId, name));
          } else {
  ...
  private static HlsMediaPlaylist parseMediaPlaylist(
    ...
    @Nullable HlsMediaPlaylist.Segment initializationSegment = null;
    HashMap<String, String> variableDefinitions = new HashMap<>();
    HashMap<String, HlsMediaPlaylist.Segment> urlToInferredInitSegment = new HashMap<>();
    List<HlsMediaPlaylist.Segment> segments = new ArrayList<>();
    List<HlsMediaPlaylist.Part> trailingParts = new ArrayList<>();
    @Nullable HlsMediaPlaylist.Part preloadPart = null;
    List<HlsMediaPlaylist.RenditionReport> renditionReports = new ArrayList<>();
    ...
    while (iterator.hasNext()) {
      ...
      if (line.startsWith(TAG_PLAYLIST_TYPE)) {
        ...
        for (int i = startIndex; i < endIndex; i++) {
          HlsMediaPlaylist.Segment segment = previousMediaPlaylist.segments.get(i);
        ...
        renditionReports.add(new HlsMediaPlaylist.RenditionReport(playlistUri, lastMediaSequence, lastPartIndex));
        ...
        @Nullable HlsMediaPlaylist.Segment inferredInitSegment = urlToInferredInitSegment.get(segmentUri);
        if (segmentByteRangeLength == C.LENGTH_UNSET) {
          // The segment has no byte range defined.
          segmentByteRangeOffset = 0;
    ...
    Map<Uri, HlsMediaPlaylist.RenditionReport> renditionReportMap = new HashMap<>();
    for (int i = 0; i < renditionReports.size(); i++) {
      HlsMediaPlaylist.RenditionReport renditionReport = renditionReports.get(i);
      ...
      if (lastPartIndex == C.INDEX_UNSET && partTargetDurationUs != C.TIME_UNSET) {
        List<HlsMediaPlaylist.Part> lastParts =
      ...
      renditionReportMap.put(
          ...
          new HlsMediaPlaylist.RenditionReport(renditionReport.playlistUri, lastMediaSequence, lastPartIndex));
    }
