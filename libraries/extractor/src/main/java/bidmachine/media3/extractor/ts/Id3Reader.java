...
public final class Id3Reader implements ElementaryStreamReader {
  ...
  public Id3Reader() {
    id3Header = new ParsableByteArray(Id3Decoder.ID3_HEADER_LENGTH);
  ...
  public void consume(ParsableByteArray data) {
    ...
    if (sampleBytesRead < Id3Decoder.ID3_HEADER_LENGTH) {
      // We're still reading the ID3 header.
      int headerBytesAvailable = Math.min(bytesAvailable, Id3Decoder.ID3_HEADER_LENGTH - sampleBytesRead);
      System.arraycopy(
          data.getData(),
          data.getPosition(),
          id3Header.getData(),
      ...
      if (sampleBytesRead + headerBytesAvailable == Id3Decoder.ID3_HEADER_LENGTH) {
        ...
        sampleSize = Id3Decoder.ID3_HEADER_LENGTH + id3Header.readSynchSafeInt();
      }
    }
