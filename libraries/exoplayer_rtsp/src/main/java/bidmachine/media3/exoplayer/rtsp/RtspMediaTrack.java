...
import static bidmachine.media3.container.NalUnitUtil.NAL_START_CODE;
...
/* package */ final class RtspMediaTrack {
  ...
  private static Uri extractTrackUri(
    ...
    if (!TextUtils.isEmpty(rtspHeaders.get(RtspHeaders.CONTENT_BASE))) {
      sessionUri = Uri.parse(rtspHeaders.get(RtspHeaders.CONTENT_BASE));
    } else if (!TextUtils.isEmpty(rtspHeaders.get(RtspHeaders.CONTENT_LOCATION))) {
      sessionUri = Uri.parse(rtspHeaders.get(RtspHeaders.CONTENT_LOCATION));
    }
