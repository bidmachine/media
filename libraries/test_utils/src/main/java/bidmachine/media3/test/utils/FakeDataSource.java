...
public class FakeDataSource extends BaseDataSource {
  ...
  @Nullable private FakeDataSet.FakeData fakeData;
  ...
  public final long open(DataSpec dataSpec) throws IOException {
    ...
    FakeDataSet.FakeData fakeData = fakeDataSet.getData(dataSpec.uri.toString());
    if (fakeData == null) {
    ...
    long totalLength = 0;
    for (FakeDataSet.FakeData.Segment segment : fakeData.getSegments()) {
  ...
  public final int read(byte[] buffer, int offset, int length) throws IOException {
    ...
    while (true) {
      FakeDataSet.FakeData fakeData = Util.castNonNull(this.fakeData);
      if (currentSegmentIndex == fakeData.getSegments().size() || bytesRemaining == 0) {
        return C.RESULT_END_OF_INPUT;
      }
      FakeDataSet.FakeData.Segment current = fakeData.getSegments().get(currentSegmentIndex);
