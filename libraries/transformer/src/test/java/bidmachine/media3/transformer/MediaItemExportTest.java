...
package bidmachine.media3.transformer;

import static bidmachine.media3.common.util.Assertions.checkNotNull;
import static bidmachine.media3.test.utils.robolectric.RobolectricUtil.runLooperUntil;
import static bidmachine.media3.transformer.AssetLoader.SUPPORTED_OUTPUT_TYPE_DECODED;
import static bidmachine.media3.transformer.AssetLoader.SUPPORTED_OUTPUT_TYPE_ENCODED;
import static bidmachine.media3.transformer.Transformer.PROGRESS_STATE_AVAILABLE;
import static bidmachine.media3.transformer.Transformer.PROGRESS_STATE_NOT_STARTED;
import static bidmachine.media3.transformer.Transformer.PROGRESS_STATE_UNAVAILABLE;
import static bidmachine.media3.transformer.Transformer.PROGRESS_STATE_WAITING_FOR_AVAILABILITY;
...
import bidmachine.media3.common.Effect;
import bidmachine.media3.common.MediaItem;
import bidmachine.media3.common.MimeTypes;
import bidmachine.media3.common.audio.AudioProcessor;
import bidmachine.media3.common.audio.SonicAudioProcessor;
import bidmachine.media3.common.util.Util;
import bidmachine.media3.effect.Presentation;
import bidmachine.media3.effect.ScaleAndRotateTransformation;
import bidmachine.media3.exoplayer.source.DefaultMediaSourceFactory;
import bidmachine.media3.exoplayer.source.MediaSource;
import bidmachine.media3.extractor.DefaultExtractorsFactory;
import bidmachine.media3.extractor.Extractor;
import bidmachine.media3.extractor.ExtractorInput;
import bidmachine.media3.extractor.ExtractorOutput;
import bidmachine.media3.extractor.ExtractorsFactory;
import bidmachine.media3.extractor.PositionHolder;
import bidmachine.media3.test.utils.DumpFileAsserts;
...
public final class MediaItemExportTest {
  ...
  private TestUtil.TestMuxerFactory.TestMuxerHolder testMuxerHolder;
  ...
  public void setUp() throws Exception {
    ...
    TestUtil.createEncodersAndDecoders();
  }

  @After
  public void tearDown() throws Exception {
    Files.delete(Paths.get(outputPath));
    TestUtil.removeEncodersAndDecoders();
  }

  @Test
  public void start_videoOnlyPassthrough_completesSuccessfully() throws Exception {
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false).build();
    MediaItem mediaItem = MediaItem.fromUri(TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_VIDEO_ONLY);

    transformer.start(mediaItem, outputPath);
    TransformerTestRunner.runLooper(transformer);

    DumpFileAsserts.assertOutput(
        context, checkNotNull(testMuxerHolder.testMuxer), TestUtil.getDumpFileName(
            TestUtil.FILE_VIDEO_ONLY));
  }
  ...
  public void start_audioOnlyPassthrough_completesSuccessfully() throws Exception {
    ...
    MediaItem mediaItem = MediaItem.fromUri(
        TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_AUDIO_UNSUPPORTED_BY_ENCODER);
    ...
    DumpFileAsserts.assertOutput(
        ...
        TestUtil.getDumpFileName(TestUtil.FILE_AUDIO_UNSUPPORTED_BY_ENCODER));
  }

  @Test
  public void start_audioOnlyTranscoding_completesSuccessfully() throws Exception {
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false)
            .setTransformationRequest(
                new TransformationRequest.Builder()
                    .setAudioMimeType(MimeTypes.AUDIO_AAC) // supported by encoder and muxer
                    .build())
            .build();
    MediaItem mediaItem = MediaItem.fromUri(TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_AUDIO_RAW);
  ...
  public void start_audioAndVideo_completesSuccessfully() throws Exception {
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false).build();
    MediaItem mediaItem = MediaItem.fromUri(TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_AUDIO_VIDEO);
  ...
  public void start_audioAndVideo_withClippingStartAtKeyFrame_completesSuccessfully()
    ...
    MediaItem mediaItem =
        new MediaItem.Builder()
            .setUri(TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_AUDIO_VIDEO_INCREASING_TIMESTAMPS_15S)
    ...
    DumpFileAsserts.assertOutput(
        context,
        checkNotNull(testMuxerHolder.testMuxer),
        TestUtil.getDumpFileName(TestUtil.FILE_AUDIO_VIDEO_INCREASING_TIMESTAMPS_15S + ".clipped"));
  }
  ...
  public void start_withSubtitles_completesSuccessfully() throws Exception {
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false)
            ...
            .build();
    MediaItem mediaItem = MediaItem.fromUri(TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_WITH_SUBTITLES);
  ...
  public void start_removeAudio_completesSuccessfully() throws Exception {
    ...
    EditedMediaItem editedMediaItem =
        new EditedMediaItem.Builder(MediaItem.fromUri(
            TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_AUDIO_VIDEO))
            .setRemoveAudio(true)
            .build();
    ...
    DumpFileAsserts.assertOutput(
        context,
        checkNotNull(testMuxerHolder.testMuxer),
        TestUtil.getDumpFileName(TestUtil.FILE_AUDIO_VIDEO + ".noaudio"));
  }
  ...
  public void start_removeVideo_completesSuccessfully() throws Exception {
    ...
    DumpFileAsserts.assertOutput(
        context,
        checkNotNull(testMuxerHolder.testMuxer),
        TestUtil.getDumpFileName(TestUtil.FILE_AUDIO_VIDEO + ".novideo"));
  }
  ...
  public void start_forceAudioTrackAndRemoveAudio_generatesSilentAudio() throws Exception {
    ...
    DumpFileAsserts.assertOutput(
        context,
        checkNotNull(testMuxerHolder.testMuxer),
        TestUtil.getDumpFileName(TestUtil.FILE_AUDIO_VIDEO + ".silentaudio"));
  }
  ...
  public void start_forceAudioTrackOnVideoOnly_generatesSilentAudio() throws Exception {
    ...
    DumpFileAsserts.assertOutput(
        context,
        checkNotNull(testMuxerHolder.testMuxer),
        TestUtil.getDumpFileName(TestUtil.FILE_VIDEO_ONLY + ".silentaudio"));
  }
  ...
  public void start_adjustSampleRate_completesSuccessfully() throws Exception {
    ...
    DumpFileAsserts.assertOutput(
        context,
        checkNotNull(testMuxerHolder.testMuxer),
        TestUtil.getDumpFileName(TestUtil.FILE_AUDIO_VIDEO + ".48000hz"));
  }
  ...
  public void start_withMultipleListeners_callsEachOnError() {
    ...
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false)
            ...
            .build();
    MediaItem mediaItem = MediaItem.fromUri(
        TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_AUDIO_UNSUPPORTED_BY_DECODER);
  ...
  public void start_withMultipleListeners_callsEachOnFallback() throws Exception {
    ...
    TransformationRequest fallbackTransformationRequest =
        new TransformationRequest.Builder().setAudioMimeType(MimeTypes.AUDIO_AAC).build();
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ true)
            .addListener(mockListener1)
            .addListener(mockListener2)
            .addListener(mockListener3)
            .build();
    MediaItem mediaItem = MediaItem.fromUri(
        TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_AUDIO_UNSUPPORTED_BY_MUXER);
  ...
  public void start_flattenForSlowMotion_completesSuccessfully() throws Exception {
    ...
    EditedMediaItem editedMediaItem =
        new EditedMediaItem.Builder(MediaItem.fromUri(
            TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_WITH_SEF_SLOW_MOTION))
    ...
    DumpFileAsserts.assertOutput(
        context,
        checkNotNull(testMuxerHolder.testMuxer),
        TestUtil.getDumpFileName(TestUtil.FILE_WITH_SEF_SLOW_MOTION));
  }
  ...
  public void start_withAudioMuxerFormatUnsupported_completesSuccessfully() throws Exception {
    ...
    DumpFileAsserts.assertOutput(
        context,
        checkNotNull(testMuxerHolder.testMuxer),
        TestUtil.getDumpFileName(TestUtil.FILE_AUDIO_UNSUPPORTED_BY_MUXER + ".fallback"));
    verify(mockListener)
        .onFallbackApplied(
  ...
  public void start_withSlowOutputSampleRate_completesWithError() {
    ...
    Muxer.Factory muxerFactory =
        new TestUtil.TestMuxerFactory(testMuxerHolder, /* maxDelayBetweenSamplesMs= */ 1);
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false)
  ...
  public void start_withUnsetMaxDelayBetweenSamples_completesSuccessfully() throws Exception {
    Muxer.Factory muxerFactory =
        new TestUtil.TestMuxerFactory(testMuxerHolder, /* maxDelayBetweenSamplesMs= */ C.TIME_UNSET);
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false)
            .setMuxerFactory(muxerFactory)
            .build();
    ...
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false)
            .setAssetLoaderFactory(
                new TestUtil.FakeAssetLoader.Factory(SUPPORTED_OUTPUT_TYPE_DECODED, sampleConsumerRef))
            .build();
    MediaItem mediaItem = MediaItem.fromUri(TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_AUDIO_VIDEO);
  ...
  public void start_withAssetLoaderNotDecodingAndDecodingNeeded_completesWithError() {
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false)
            .setAssetLoaderFactory(
                new TestUtil.FakeAssetLoader.Factory(
  ...
  public void start_withOnlyRegularRotationEffect_transmuxesAndRotates() throws Exception {
    ...
    DumpFileAsserts.assertOutput(
        context,
        checkNotNull(testMuxerHolder.testMuxer),
        TestUtil.getDumpFileName(TestUtil.FILE_AUDIO_VIDEO + ".rotated"));
  }
  ...
  public void getProgress_unknownDuration_returnsConsistentStates() throws Exception {
    Transformer transformer =
        TestUtil.createTransformerBuilder(testMuxerHolder, /* enableFallback= */ false).build();
    MediaItem mediaItem = MediaItem.fromUri(
        TestUtil.ASSET_URI_PREFIX + TestUtil.FILE_UNKNOWN_DURATION);
    AtomicInteger previousProgressState =
        new AtomicInteger(PROGRESS_STATE_WAITING_FOR_AVAILABILITY);
    AtomicBoolean foundInconsistentState = new AtomicBoolean();
    Handler progressHandler =
