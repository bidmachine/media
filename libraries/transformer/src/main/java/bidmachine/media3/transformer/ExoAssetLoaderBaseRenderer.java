...
import static bidmachine.media3.decoder.DecoderInputBuffer.BUFFER_REPLACEMENT_MODE_DISABLED;
...
/* package */ abstract class ExoAssetLoaderBaseRenderer extends BaseRenderer {
    ...
    if (inputFormat == null) {
      ...
      shouldInitDecoder =
          assetLoaderListener.onTrackAdded(
              inputFormat, AssetLoader.SUPPORTED_OUTPUT_TYPE_DECODED | AssetLoader.SUPPORTED_OUTPUT_TYPE_ENCODED);
    }

    if (shouldInitDecoder) {
      if (TransformerUtil.getProcessedTrackType(inputFormat.sampleMimeType) == C.TRACK_TYPE_VIDEO) {
  ...
  private boolean ensureSampleConsumerInitialized() throws ExportException {
    ...
    if (outputFormat == null) {
      if (decoder != null
          && TransformerUtil.getProcessedTrackType(inputFormat.sampleMimeType) == C.TRACK_TYPE_AUDIO) {
